...
A small, personal sideload Android app that visually masks the LINE VOOM promotional banner in LINE's top-level **Chats** tab and permanently covers the final three sections of LINE's five-section bottom navigation. It supports both the original navigation and the redesigned version that replaces **Voom** with **Shopping**. It uses an accessibility service restricted to `jp.naver.line.android`; it does not click the banner, modify LINE, store content, or transmit data.
